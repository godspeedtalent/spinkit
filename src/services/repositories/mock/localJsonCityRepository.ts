// src/services/repositories/mock/localJsonCityRepository.ts
...
import allGenresJson from '@/data/mock-data/json/genres.json'; // Import as array of genre objects
...
const allGenres: { name: string }[] = allGenresJson.map(g => ({ name: g.name })); // Assuming genres.json is an array of objects with a name property

export class LocalJsonCityRepository implements ICityRepository {
  private djs: DJ[] = djsData as DJ[];
  private venues: Venue[] = venuesData as Venue[];
  private allGenresList: string[] = allGenres.map(g => g.name);
  ...
  async getCityDetails(cityName: string): Promise<CityDetails | null> {
    ...
    let aiHintForCityImage = cityObject?.aiHint || cityName.toLowerCase().replace(/\s+/g, '-');
    ...
    return {
      ...
      profilePicSeed: cityObject?.profilePic || aiHintForCityImage,
