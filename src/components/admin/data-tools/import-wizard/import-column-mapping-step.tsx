...
import { DO_NOT_IMPORT_VALUE, CREATE_NEW_FIELD_VALUE, NEW_FIELD_TYPE_OPTIONS, TARGET_MONGO_FIELDS_MAP, getEntityTypeLabel } from '@/config/data-import-config';
...
const ImportColumnMappingStep: React.FC<ImportColumnMappingStepProps> = ({
  ...
  if (!mappedSourceTables || mappedSourceTables.length === 0) {
    return <p className="text-muted-foreground p-4 text-center">No tables mapped for column configuration. Please complete Table Mapping.</p>;
  }
  ...
  return (
    <WizardStepSection
      ...
      icon={ListChecks}
      description="For each mapped table, map its source columns to the target MongoDB fields."
    >
      <Button variant="outline" size="xs" onClick={onResetColumnMappings} className="text-xs mb-3" disabled={isProcessingMappings}>
      ...
      <Tabs defaultValue={mappedSourceTables[0]} className="w-full">
        ...
        {mappedSourceTables.map((sourceTable) => {
          ...
          const currentTargetResourceKey = targetConnectionInfo?.collections
            ...
            : undefined;
          const currentTargetFields = (currentTargetResourceKey ? (TARGET_MONGO_FIELDS_MAP[currentTargetResourceKey as keyof typeof TARGET_MONGO_FIELDS_MAP] || []) : []);
              ...
              <Card className="shadow-inner bg-background">
                <CardHeader className="p-3 border-b bg-muted/30">
                  <h4 className="text-sm font-semibold">
                    ...
                    <span className="text-xs text-muted-foreground"> ({getEntityTypeLabel(currentTargetResourceKey || "unknown" as any)})</span>
                  </h4>
                </CardHeader>
      ...
      <WizardStepNavigation
        ...
        nextLabel={isProcessingMappings ? "Applying..." : "Apply Mappings & Preview"}
